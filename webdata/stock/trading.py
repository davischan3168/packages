...
import os
from webdata.stock.reference import new_stocks
import string
try:
...
def get_tick_data(code=None, date=None, retry_count=3, pause=0.001,
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            #re = Request(ct.TICK_PRICE_URL % (ct.P_TYPE['http'], ct.DOMAINS['sf'], ct.PAGES['dl'],date, symbol))
    ...
    if ktype.upper() in ct.K_LABELS:
        ...
        if (start is None or start == '') & (end is None or end == ''):
            ...
            for year in years:
                ...
                url = ct.KLINE_TT_URLL%(ct.P_TYPE['http'], ct.DOMAINS['tt'],
            ...
            try:
                ...
                if len(js['data'][symbol][dataflag][0]) == 6:
                    ...
                    try:
                        ...
                        pass
                    df.columns = ct.KLINE_TT_COLS_MINS
                else:
                    df = pd.DataFrame(js['data'][symbol][dataflag], 
